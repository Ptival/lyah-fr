...
la fonction, puis une espace, puis ses paramètres, séparés par des espaces. Par
...
paramètre par une espace. Appeler une fonction avec plusieurs paramètres est
...
L'application de fonction (appeler une fonction en mettant une espace après
puis ses paramètres) a la plus grande des précédences. Cela signifie pour nous
...
désirez combiner, et son type dépend du nombre de composantes qu'il a et du
type de ces composantes. Ils sont dénotés avec des parenthèses, et les
composantes sont séparées par des virgules.
...
(aussi appelé un couple) est un type propre, ce qui signifie qu'une liste ne
peut pas avoir quelques couples puis un triplet (tuple de taille trois),
...
Cela nous dit que l'on a essayé d'utiliser un couple et un triplet dans la même
liste, ce qui ne doit pas arriver. Vous ne pourriez pas non plus créer une
liste `[(1, 2), ("One", 2)]` car le premier élément de cette liste est un
couple de nombres alors que le second est un couple d'une chaîne de caractères
...
un couple, une fonction pour ajouter à un triplet, un fonction pour ajouter à
un quadruplet, etc.
...
tailles différentes. Deux fonctions utiles qui opèrent sur des couples&nbsp;:
...
**Note&nbsp;:** ces deux fonctions opèrent seulement sur des couples. Elles ne
...
Une fonction cool qui produit une liste de couples&nbsp;: `zip`. Elle prend
deux listes et les zippe ensemble en joignant les éléments correspondants en
des couples. C'est une fonction très simple, mais elle sert beaucoup. C'est
...
Elle couple les éléments et produit une nouvelle liste. Le premier élément va
avec le premier, le deuxième avec le deuxième, etc. Remarquez que, puisque les
couples peuvent avoir différents types en elles, `zip` peut prendre deux listes
qui contiennent des éléments de différents types et les zipper. Que se
passe-t-il si les longueurs des listes ne correspondent pas ?
