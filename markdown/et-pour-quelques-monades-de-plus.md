...
`mappend` les unes aux autres et ceci se reflète dans le résultat final. Voici
...
comme des valeurs avec un contexte, et ainsi imaginer ce que notre résultat
devrait être.
...
Quand vous utilisez la monade `Writer`, il faut être extrêmement prudent avec
...
fonction. Ce qui se passe ici, c'est qu'elle prend un nombre, puis fait `(*2)`
...
Haskell est un langage pur, et grâce à cela, nos programmes sont faits de
...
parfois être fastidieux à modéliser. C'est pourquoi Haskell offre la monade
...
calculs à états, on devrait se débarrasser de la pureté d'Haskell. Eh bien, cela
...
Si un type est à la fois instance de `Functor` et de `Monad` et obéit leurs
...
On donne la valeur monadique `m` à la fonction, et on applique `f` à son
...
donc ils sont joints en un gros calcul réussi. La deuxième ligne contient un
...
Le valeur monoïdale extérieure `"bbb"` vient d'abord, puis `"aaa"` est
...
Peut-être que la chose la plus intéressante à propos de `join` est que,
pour chaque monade, donner une valeur monadique à une fonction avec `>>=`
est équivalent à mapper cette fonction sur la valeur puis utiliser `join`
pour aplatir la valeur monadique imbriquée résultante ! En d'autres termes,
`m >>= f` est toujours égal à `join (fmap f m)` ! C'est logique quand on y
réfléchit. Avec `>>=`, on donne une valeur monadique à une fonction qui
attend une valeur normale et retourne une valeur monadique. Si l'on mappe
cette fonction sur la valeur monadique, on a une valeur monadique à
l'intérieur d'une valeur monadique. Par exemple, si l'on a `Just 9` et la
fonction `\x -> Just (x + 1)`. En mappant la fonction sur `Just 9`, on
obtient `Just (Just 10))`.
...
Les trois premiers cas sont comme les anciens, sauf que la nouvelle pile est
...
s'aperçoit que ce type représente une valeur dans un contexte et peut agir
...
moitiés, ce qui nous donne un tout, et à la troisième ligne on additionne un
...
atterrissent toutes sur pile ? D'abord, créons des valeurs de probabilités pour
...
avec elle. Je trouve ça plutôt attrayant ! À ce stade, on devrait avoir une
