...
<img src="img/pattern.png" alt="quatre !" class="right"/>
...
**Note&nbsp;:** Le motif `x:xs` est très utilisé, particulièrement dans les
...
petit ou égal à `18.5`. Puisque ce n'est pas le cas, elle passe à la prochaine
...
sont évaluées à `False` (donc, qu'on n'a pas fourni de garde `otherwise` qui
vaut toujours `True`), l'évaluation passe au **motif** suivant. C'est ainsi que
les motifs et les gardes s'entendent bien. Si aucune combinaison de gardes et
de motifs n'est satisfaite, une erreur est levée.
