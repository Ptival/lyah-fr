...
<img src="img/knight.png" alt="ahahahaha !" class="left"/>
...
Les classes de types en Haskell sont utilisées pour présenter une interface
pour des types qui partagent un comportement. On a commencé avec des classes de
...
<img src="img/smug.png" alt="satisfait comme jamais" class="left"/>
...
Le module `Data.Monoid` exporte deux types pour cela, `Product` et `Sum`.
...
*ou* avec `False`, et `True` en faisant *ou* avec `True`. Le *newtype* `Any`
...
n'importe laquelle (NDT&nbsp;: en anglais, *any*) de ses valeurs est `True`.
Même si trois ou plus `Bool` enveloppés dans `Any` sont `mappend` ensemble, le
résultat sera toujours `True` lorsque n'importe lequel d'entre eux est
`True`&nbsp;:
...
si toutes (NDT&nbsp;: en anglais, *all*), les valeurs passées à `mappend` sont
`True`&nbsp;:
...
monoïdes est juste l'affaire de regarder ce qu'on utilisait déjà afin de voir
si ces choses présentent un comportement monoïdal. Pour `Ordering`, il faut y
regarder à deux fois avant de reconnaître le monoïde, mais il s'avère que son
instance de `Monoid` est tout aussi intuitive que celles recontrées
...
<img src="img/bear.png" alt="quelqu'un a ORDONNÉ que des pizzas soient livrées
?!?" class="right"/>

L'instance est créée de la sorte&nbsp;: quand on `mappend` deux valeurs
`Ordering`, celle de gauche est préservée, à moins que la valeur à gauche soit
`EQ`, auquel cas celle de droite est le résultat. Le neutre est `EQ`. Au
départ, ça peut sembler arbitraire, mais cela ressemble en fait beaucoup à la
façon donc on compare lexicographiquement des mots. On compare d'abord les
premières lettres de chaque mot, et si elles diffèrent, on peut immédiatement
décider de l'ordre des mots dans un dictionnaire. Cependant, si elles sont
égales, alors on doit comparer la prochaine paire de lettres et répéter le
procédé.
...
OK, comment est-ce que ce monoïde est utile ? Mettons que vous soiyez en train
...
valeurs `Just`, le contenu des `Just` est `mappend` et est enveloppé de nouveau
...
*newtype* `First`. Si le premier paramètre de `mappend` est une valeur `Just`,
...
savoir s'il y a un `Just` dans le tas. La fonction `mconcat` s'avère
utile&nbsp;:
...
des plis sur presque toute structure de données. Les arbres se prêtent
...
exporte des fonctions dont les noms sont en collision avec ceux du `Prelude`,
elle est préférablement importée qualifiée (et servie avec du basilic)&nbsp;:
...
Vous rappelez-vous la structure de données arborescente du chapitre [Créer nos
...
<img src="img/accordion.png" alt="je vous laisse deviner cette blague visuelle"
class="right"/>

On pense ainsi&nbsp;: si l'on nous donne une fonction qui prend un élément de
notre arbre et retourne une valeur monoïdale, comment réduit-on notre arbre à
une simple valeur monoïdale ? Quand nous faisions `fmap` sur notre arbre, on
...
la valeur du nœud, suivi du sous-arbre droit.

Remarquez qu'on n'a pas eu besoin d'écrire la fonction qui prend une valeur et
...
Maintenant qu'on a une instance de `Foldable` pour notre type arborescent, on a
`foldr` et `foldl` gratuitement ! Considérez cet arbre&nbsp;:
...
également à réduire une structure à une simple valeur monoïdale. Par exemple,
...
`foldMap` avec la fonction `\x -> [x]`. En projetant d'abord la fonction sur
